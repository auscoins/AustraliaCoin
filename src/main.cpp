...
uint256 hashGenesisBlock("0x7e6ca6d1eed976138b851c29c99afbb2f9758e9b3df514afd2a85ba260328c9b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // AUSCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "AUSCoin Signed Message:\n";
...
{
    // AUSCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 2100000 blocks, which will occur approximately every 2 years
    nSubsidy >>= (nHeight / 2250000); // Auscoin: 2100000 blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 60; // AUSCoin: 3.5 days
static const int64 nTargetSpacing = 1 * 30; // AUSCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // AUSCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // AUSCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // AUSCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7e6ca6d1eed976138b851c29c99afbb2f9758e9b3df514afd2a85ba260328c9b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Once a jolly swagman, camped by a billabong. Under the shade of a coolabah tree.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1396713309;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1376254;
        ...
        assert(block.hashMerkleRoot == uint256("0x1e9177e1d5654dc728948dcc8c33189d4ffad47143452a665bf89952f67cd022"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xf0, 0xf6, 0xfb }; // AUSCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AUSCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AUSCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AUSCoinMiner(CWallet *pwallet)
{
    printf("AUSCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("auscoin-miner");
    ...
    try { loop {
        ...
        printf("Running AUSCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AUSCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AUSCoinMiner, pwallet));
}
